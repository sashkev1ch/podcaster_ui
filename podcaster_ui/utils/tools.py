...
def get_rss_channel(url: str) -> Dict[str, str]:
    response = get(url)
    if response.status_code != 200:
        raise RssParseException(f"Can't reach rss url {url}, returned status code: {response.status_code}")
...
def get_rss_data(channel: Channel) -> List[Dict[str, Union[str, int, datetime, Channel]]]:
    response = get(channel.url)
    if response.status_code != 200:
        raise RssParseException(f"Can't reach rss url {channel.url}, returned status code: {response.status_code}")
    rss = RSSParser.parse(response.text)
